...
if($_POST['senha']=='boo@happy2014'){
  ...
  $result = mysql_query($sql);
  $conteudo = utf8_encode('<p>Abaixo seu e-ticket, siga as instru&ccedil;&otilde;es a seguir para saber como realizar a troca pelo ingresso principal que dar&aacute; acesso ao evento.</p>');
  while(list($id_item,$nome_evento,$quantidade)=mysql_fetch_row($result)){
    ...
    for($i=0;$i<$quantidade;$i++){
      ...
      $conteudo.= utf8_encode('<div style="margin: 3px; font-family: arial; font-size: 14; padding: 6px; border: 1px black solid">
  ...
  $conteudo .= utf8_encode('<p>'.$informacoes.'</p>
  ...
  //if(mail($email,"E-ticket Ingresso Solidário",$conteudo,$headers))
    //echo "<h3>Tickets enviados com sucesso";
  
  //if(mail('pedrodemenezes.tec@gmail.com',"E-ticket Ingresso Solidário",$conteudo,$headers))
    //echo "<h3>Tickets enviados com sucesso";
  ...
  echo '<br><a href="emite_ticket.php">Voltar</a>';
}
?>