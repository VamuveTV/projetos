	...
	$("#browser").treeview({collapsed: true});
		...
		?>
	        ...
	        if($("form")[0].checkValidity()) {  //verificando se o form foi validado
				$('.loader').show();
				$.ajax({//método ajax
					url: "ajax_editar_varios.php", //página requisitada
					type: "POST",
					data: {'produtos[]' : produtos,'nome' : nome, 'classificacao': classificacao, 'descricao' : descricao, 'unidade[]' : unidade, 'cor': cor, 'tamanho': tamanho, 'frete': frete, 'lancamento': lancamento, 'peso': peso, 'referencia': referencia, 'imposto': imposto, 'marca': marca, 'categorias[]': categorias},
