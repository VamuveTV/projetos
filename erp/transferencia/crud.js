...
function chamaTabela2(codigo) {
    ...
    var origem = $('#buscaOrigem').val();
    var destino = $('#buscaDestino').val();
    var produto = $('#buscaProduto').val();

    $.ajax({//método ajax
        url: "tabeladados.php?page="+codigo, //página requisitada
        type: "POST",
        data: {cod: cod, data: data, origem: origem, destino: destino, produto: produto},
...
function chamaAutorizar() {
	$.ajax({//método ajax
		url: "autorizar.php", //página requisitada
		success: function(retorno){ //em caso de sucesso
			$("#entry").hide().html(retorno).fadeIn(1000);
		}
	}); 
...
function autoriza(codigo) {
    var obs = $('#obs_'+codigo).val();

    $.ajax({//método ajax
        url: "ajax_autoriza.php", //página requisitada
        type: "POST",
        data: {cod: codigo, obs: obs},
        success: function(retorno){ //em caso de sucesso
            $("#entry").hide().html(retorno).fadeIn(1000);
            chamaAutorizar();
        }
...
function nao_autoriza(codigo) {
    var obs = $('#obs_'+codigo).val();

    $.ajax({//método ajax
        url: "ajax_nao_autoriza.php", //página requisitada
        type: "POST",
        data: {cod: codigo, obs: obs},
        success: function(retorno){ //em caso de sucesso
            $("#entry").hide().html(retorno).fadeIn(1000);
    ...
    excluir(codigo);
		...
		$(document).ready(function(){
			...
			$('#cadastrar').live ('click',function(){
				...
				var destino		=	$('#unidade-destino').val();
                ...
                var observacao	=	$('#observacao').val();
                    ...
                    if($("form")[0].checkValidity()) {  //verificando se o form foi validado
						...
						$.ajax({//método ajax
							...
							type: "POST",
							data: {destino : destino, produto: produto, quantidade : quantidade, observacao: observacao},
							success: function(retorno){ //em caso de sucesso
								fecha();
                               	$('.log').html('<div class="alert alert-info">'+retorno+'</div>');
								chamaTabela(1);
				...
				click: function(){
                    ...
                    if($("form")[0].checkValidity()) {  //verificando se o form foi validado
                        $.ajax({//método ajax
                            ...
                            type: "POST",
                            data: {cod: cod, nome : nome, login: login, senha : senha, unidade: unidade},
                            success: function(retorno){ //em caso de sucesso
                                fecha();
                                $('.log').html('<div class="alert alert-info">'+retorno+'</div>');
                                chamaTabela(1);
