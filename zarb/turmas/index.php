...
$campos = array("curso","periodo","turno","carga","pagamento","observacao","status");//nome dos campos na tabela. 
$dados = array($_POST['curso'],$_POST['periodo'],$_POST['turno'],$_POST['carga'],$_POST['pagamento'],$_POST['observacao'],$_POST['status']);//nome dos campos do form, devem seguir ordem dos campos da tabela
$conn = new ZarbMysql;
//-------------------------------------------------------------------------
...
else if($_GET["param"]=="new" || $_GET["cod"])
{
  ...
  if($cod){
    $fazer="Salvar";
    $sql = "SELECT codigo, curso, periodo, turno, carga, pagamento, observacao, status FROM $tabela WHERE codigo='$cod' ";
    $res = mysql_query($sql);
    list($codigo, $curso, $periodo, $turno, $carga, $pagamento, $observacao, $status)=mysql_fetch_row($res);
  }else
    $fazer="Gravar";
?>
